...
namespace Game.Modules
{
    public sealed class WeaponsModule : Module<WeaponsModuleView>
    {
        ...
        private readonly Dictionary<BulletControllerType, Type> _weaponsControllersMap;

        public WeaponsModule(WeaponsModuleView view): base(view)
        {
            _weaponsControllersMap = new Dictionary<BulletControllerType, Type>
            {
        ...
        public override void Initialize()
        {
            ...
            var model = new WeaponModel(config, index, serial, level);
        ...
        private void OnWeaponSelected(WeaponModel model, UnitController unit)
        {
            ...
            {
                var bulletsCount = weaponExisted.Model.BulletsCount;
                bulletsCount = Mathf.Min(bulletsCount + 1, GameConstants.BulletsMax);
                weaponExisted.Model.BulletsCount = bulletsCount;
                return;
            }

            var count = _levelManager.WeaponsMap.Count;
            if (count >= GameConstants.WeaponsMax)
            {
                Log.Info("Weapon limit reached!");
                return;
            }
        ...
        private IEnumerator SpawnBulletsCoroutine(WeaponController weapon, float delay)
        {
            ...
            for (int i = 0; i < bulletsCount; i++)
            {
                ...
                var model = new BulletModel(bulletType, bulletIndex, i, bulletsCount, moveSpeed, lifeTime, range, damage, orbitRadius, unit.TeamID);
        ...
        private void OnBossSpawned(EnemyController boss)
        {
            ...
            {
                var model = new WeaponModel(config, config.Index, 0, 0);
